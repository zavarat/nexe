...
let DOWNLOAD_DIR;
 ...
 module.exports = class Patch {
   ...
   node(version, done) {
      ...
      * @param {function} monkeyPatched - function to find location.
      * @param {function} processor - function r=to process contents
     ...
     let _monkeypatch = function(filePath, monkeyPatched, processor, complete) {
       async.waterfall([
         ...
         (content, next) => {
           ...
           console.log('monkey patch', filePath);
     ...
     async.waterfall([
       /**
        * Patch node for nexe child_process support. (better)
        **/
       (next) => {
         let childProcPath = path.join(outPath, 'lib', 'child_process.js');

         console.log('nexe', 'patch child_process');
         _monkeypatch(
             childProcPath,
             function (content) {
       ...
       /**
        * Fix for src/node.js -> lib/internal/bootstrap_node.js
        **/
       (next) => {
         ...
         console.log('nexe', 'patch node.js')
         _monkeypatch(
           mainPath,
           function (content) {
       ...
       /**
        * Patch node.gyp to include nexe files.
        */
       (next) => {
         let gypPath = path.join(outPath, 'node.gyp');

         console.log('nexe', 'patch node.gyp')
         _monkeypatch(
           gypPath,
           function (content) {
       ...
       (next) => {
         ...
         let snapshotPath    = this.config.startupSnapshot;

         if (snapshotPath) {
           console.log('v8-snapshots', 'patch configure')
