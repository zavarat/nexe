...
module.exports = class Download {
  ...
  extractNode(next) {
    ...
    let done   = (compout, ver) => {
      if(ver === 'latest') {
        fs.readdirSync(outPath).forEach(file => {
          ...
          console.log('extractNode:latest', compout)
        })
      }
    ...
    if(fs.existsSync(computedOut) || version === 'latest' && fs.existsSync(outPath)) {
      console.log('extract', 'out already exists', outPath);
